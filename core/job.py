...
# RECONFIGURADO (2026-08-31): mesma ideia de _modalidade_pelo_titulo, mas
# lendo `local` em vez de `titulo` — checados SEPARADAMENTE (nunca
# concatenados, ver o MEDIDO em _confirma_remoto sobre esse bug original).
# Motivado por um caso real: "SAP Master Data Specialist for Finance"
# (Scandinavian Tobacco Group) veio com modalidade="Remoto" via f_WT=2 do
# LinkedIn, mas é Híbrido de verdade (3 dias/semana em Alcântara, Lisboa) —
# nem título nem `local` ("Lisboa, Lisboa, Portugal") continham a palavra
# "híbrido", então nem esta checagem nem a de título pegam ESTE caso
# específico (a badge de modalidade só existe na página individual da
# vaga, que o scraper de busca não abre). Vale como rede de segurança pra
# quando `local` VIER com o termo explícito (visto em outras fontes/
# formatos), não como garantia de pegar todo falso positivo do f_WT=2.
def _modalidade_pelo_local(local: str) -> str | None:
    """Devolve a modalidade real quando `local` contradiz "remoto"
    (Híbrido/Presencial) — None quando não contradiz nada."""
    local_norm = _normalizar(local)
    if any(termo in local_norm for termo in _TERMOS_TITULO_HIBRIDO):
        return "Híbrido"
    if any(termo in local_norm for termo in _TERMOS_TITULO_PRESENCIAL):
        return "Presencial"
    return None
...
# RECONFIGURADO (2026-08-31, perfil do usuário): projeto herdado
# de outra usuária que buscava Júnior/Pleno — pesos originais davam bônus
# pra esse par e deságio (-2) pra Sênior/Especialista/Liderança. Perfil
# atual é sênior (12+ anos, Master Data/Governança/SAP, Product Owner,
# Coordenador) — invertido: Sênior/Especialista/Liderança agora é o alvo
# (bônus), Júnior/Pleno passa a ser "abaixo do alvo" (deságio). Estágio/
# Trainee continua neutro-baixo (fora do escopo dos dois grupos). Nada
# disso é filtro — a vaga ainda notifica, só muda a posição no ranking
# (imediata vs. digest, topo vs. fundo).
...
class Job:
    ...
    motivo: str = ""
    # RECONFIGURADO (2026-08-31): descrição completa da vaga (requisitos/
    # responsabilidades), quando a fonte expõe isso no próprio card de
    # busca sem precisar de request extra (ex: SimplifyScraper) — "" pras
    # fontes que só trazem título/empresa/local (LinkedIn, Gupy, Catho...).
    # Não entra em combina_com()/pontuar_relevancia(): é conteúdo pra
    # exibir/exportar (ver scripts/export_vagas.py), não sinal de filtro —
    # texto livre de descrição tem risco alto de falso positivo/negativo
    # se virasse critério de aprovação.
    descricao: str = ""
    # Resumo em linguagem natural gerado por core/ia_resumo.py::gerar_resumo
    # — só preenchido pras vagas de notificação imediata (ver main.py), e só
    # quando ANTHROPIC_API_KEY está configurada; "" no resto dos casos. Não
    # entra em combina_com()/pontuar_relevancia() pelo mesmo motivo de
    # `descricao`: é explicação pra exibir, nunca sinal de filtro/score.
    resumo_ia: str = ""

    def __post_init__(self):
        """Sobrepõe modalidade="Remoto" quando o TÍTULO ou o LOCAL contradiz
        (Híbrido/Presencial) — ver _modalidade_pelo_titulo/_modalidade_pelo_local.
        ...
        RECONFIGURADO (2026-08-31): título e local checados SEPARADAMENTE
        (nunca concatenados — ver o MEDIDO em _confirma_remoto), primeiro
        um, depois o outro, qualquer um dos dois que contradisser vence.
        Não pega todo falso positivo do f_WT=2 do LinkedIn — a badge real
        de modalidade só existe na página individual da vaga, que o
        scraper de busca não abre (ver caso real documentado em
        _modalidade_pelo_local) — mas amplia a cobertura pra quando o
        sinal aparecer no campo de local, não só no título.
        """
        modalidade_real = _modalidade_pelo_titulo(self.titulo) or _modalidade_pelo_local(self.local)
