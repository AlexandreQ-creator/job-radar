...
_SCRAPERS_BR = [
    ...
    # Indeed SAIU de novo (2026-09-08). Histórico: tinha saído em 18/08 por
    # bloqueio anti-bot total (medido de IP de nuvem/GitHub Actions),
    # religado em 02/09 depois de retestar local e voltar a trazer vaga real.
    # Rodou 3 dias em FREQUENCIA_BAIXA medindo rendimento líquido de verdade
    # (não simulado):
    #
    #   02/09 — 48 brutas → 22 filtradas → 0 novas
    #   03/09 — 112 brutas → 48 filtradas → 0 novas
    #   08/09 — 64 brutas → 47 filtradas → 0 novas
    #
    # Zero vaga líquida nova nos 3 dias — a fonte funciona (não é bloqueio,
    # as filtradas eram vaga real), só que sempre duplicava o que LinkedIn/
    # Gupy já tinham achado. Decisão do usuário em 08/09: desligar (mesmo
    # critério de permanência usado com as outras fontes BAIXA deste
    # perfil — sem vaga relevante líquida numa janela de dias, sai). Código
    # e import continuam no lugar; pra religar de novo no futuro (ex.: se o
    # catálogo do Indeed mudar), descomente esta linha e a equivalente em
    # _SCRAPERS_INTL.
...
_SCRAPERS_INTL = [
    ...
    # Indeed Intl SAIU de novo (2026-09-08) — ver histórico completo em
    # _SCRAPERS_BR. Rodou 3 dias em FREQUENCIA_BAIXA:
    #
    #   02/09 — 678 brutas → 46 filtradas → 7 novas (única estreia boa)
    #   03/09 — 980 brutas → 89 filtradas → 0 novas
    #   08/09 — 1121 brutas → 250 filtradas → 0 novas
    #
    # Depois do pico de 7 no primeiro dia (jobs que já circulavam noutra
    # fonte, só novos pro dedup do Indeed), zerou nos 2 ciclos seguintes —
    # e continuou sendo o fator que mais estica o ciclo Internacional
    # (~40-60min extra por causa de timeout de 60s em domínio/termo sem
    # resultado, somando os 8 domínios). Decisão do usuário em 08/09:
    # desligar junto com o Indeed BR. Pra religar: descomente esta linha e
    # a equivalente em _SCRAPERS_BR.
    ...
    # RECONFIGURADO (2026-08-31, perfil do usuário): adicionado a
    # pedido do usuário. FREQUENCIA_BAIXA por ser fonte nova, sem medição
    # de rendimento em produção ainda — mesmo critério aplicado ao
    # WeWorkRemotely quando entrou no perfil Brasil. Ajustar pra ALTA
    # depois de medir (ver scrapers/simplify.py pro que já foi validado ao
    # vivo: resultado global numa busca só, modalidade e data reais por
    # vaga — potencialmente a fonte mais rica do pipeline internacional).
    DefinicaoScraper(SimplifyScraper, FREQUENCIA_BAIXA),
]
