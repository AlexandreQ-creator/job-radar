...
def _linha_resumo_ia(job) -> str:
    """Fase 3 do TODO histórico deste arquivo: resumo em linguagem natural
    (core/ia_resumo.py::gerar_resumo, chamado em main.py antes de notificar)
    explicando por que a vaga bateu com o perfil. "" quando a IA está
    desligada (sem ANTHROPIC_API_KEY) ou falhou — nesse caso a mensagem sai
    igual a antes, só com Motivo (regra) e sem essa linha extra."""
    if not job.resumo_ia:
        return ""
    return f"<b>Por que essa vaga:</b> {job.resumo_ia}\n"
